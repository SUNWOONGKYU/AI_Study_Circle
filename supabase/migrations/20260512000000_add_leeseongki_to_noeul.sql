-- 1회차(event=3) 노을(slot=29)에 이성기(구글 가입 회원) 추가
-- 안전장치: profiles에서 '이성기' 매칭 건수 NOTICE로 확인 후 추가
...
BEGIN
    ...
    RAISE NOTICE '이성기 매칭 profiles: % 건', v_match_count;

    IF v_match_count = 0 THEN
        RAISE EXCEPTION '이성기를 profiles에서 찾을 수 없습니다.';
    ...
    ELSE
        SELECT id INTO v_user_id FROM profiles WHERE name = '이성기';
    END IF;
